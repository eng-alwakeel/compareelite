...
1. READ the articles index FIRST:
   https://raw.githubusercontent.com/eng-alwakeel/compareelite/main/data/articles-index.md

2. CHECK: Is your topic already covered?
   - If YES → choose different topic
   - If NO → proceed to write

3. FOR `related_articles`:
   - Only use slugs from the index
   - Copy the exact slug from the index
   - Never invent slugs

4. FOR topic selection:
   - Choose topics NOT in the "Topics to AVOID" list
   - Aim for topics that complement existing ones

The index is regenerated by GitHub Actions every time an article is
published, so the URL above always reflects the live site.
