...
**STEP 1a — Refresh the dedup source first (do NOT trust the committed copy).**
The committed `data/articles-slugs.txt` is only auto-regenerated by CI on pushes to `main`. A routine firing may run on a working branch or stale checkout where the file is behind (this caused 3 wasted runs in COM-444/COM-447 — a 44-entry slugs.txt while `articles/` held 200+ files). Always regenerate from the real files before reading:

```bash
node scripts/generate-articles-index.js          # rewrites data/articles-index.md from articles/*.json
ls articles/*.json | xargs -n1 basename | sed 's/\.json//' | sort > data/articles-slugs.txt
```

**STEP 1b — Pick 1 NEW topic using NORMALIZED matching.**
Dedup against the freshly regenerated list by ROOT keyword and hyphenation variant, not exact slug: `smartwatch` == `smart-watch`, `soundbar` == `sound-bar`, `wifi` == `wi-fi`. (Exact-slug matching let `best-smartwatches-2026` slip past `best-smart-watches-2026`.) With `grep -E`, use a plain `|` for OR — `\|` matches a literal pipe and silently returns 0.

Choose from whichever category is currently thinnest, rotating through:

Category rotation priority:
...
Hard rule: never pick a slug already present in the regenerated slugs.txt. If you do, the Editor will refuse with `DUPLICATE_TOPIC` — that's a wasted run.
