...
### 1a. Inject `related_articles` (CTO-only authority)

The CMO and QC do not populate `related_articles` — that responsibility
sits exclusively with the CTO because only the CTO has the read access and
the trust to author internal links.

If the article does not yet have `related_articles`:

1. Read `data/articles-manifest.json` (local file, regenerated by
   `update-manifest.yml` on every push) and parse its `slugs` array.
2. Filter that list to slugs **in the same `category`** as the new article.
   If fewer than two same-category siblings exist, fall back to adjacent
   categories (Tech ↔ Smart Home, Home Office ↔ Home Fitness).
3. Pick **2–3 slugs**. Skip the article's own slug. Skip any slug that
   already appears in `data/broken-amazon-links.json` with `state: "DEAD"`
   on more than half of its products — those are themselves quarantined.
4. For each chosen slug, read `articles/<slug>.json` to grab the real
   `title`. Build the array:
   ```json
   "related_articles": [
     { "slug": "<picked-slug-1>", "title": "<title-1>" },
     { "slug": "<picked-slug-2>", "title": "<title-2>" }
   ]
   ```
5. Write the updated JSON back to `articles/<slug>.json`.

If the published-slug list is empty (brand-new site, no siblings yet),
omit the field entirely — the renderer skips the section gracefully and
the validator does not require it.

After injection, re-run `node scripts/validate-article.js articles/<slug>.json`
to confirm the new field validates. Then proceed to step 2.
