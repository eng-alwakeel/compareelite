...
// Slugify a category into its hub-page path segment:
//   "Home Fitness" → "home-fitness", "Smart Home" → "smart-home", "Tech" → "tech"
// Falls back to "tech" for a missing/unknown category so the breadcrumb always
// points at a real hub page (never the broken /blog#Category anchor).
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: category || 'Guide',
        item: `${SITE_URL}/blog/${categoryToSlug(category)}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
...
function generateArticleSchema(article) {
  ...
  const authorName = article.author || ORGANIZATION_NAME;
  const author = {
    '@type': 'Person',
    name: authorName,
    ...(article.author_bio  ? { description: article.author_bio }       : {}),
    ...(article.author_url  ? { url: article.author_url, sameAs: [article.author_url] } : {}),
  };
