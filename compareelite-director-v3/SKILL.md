...
The committed `data/articles-index.md` / `data/articles-slugs.txt` are only auto-regenerated by CI on pushes to `main`. A routine firing may run on a working branch where they are stale (this caused wasted picks in COM-444 — a 44-entry index while `articles/` held 200+ files). Always regenerate from the real files before reading:
...
**STEP 1b — Pick NEW topics using NORMALIZED matching.** Dedup against the freshly regenerated list by ROOT keyword and hyphenation variant, not exact slug: `smartwatch` == `smart-watch`, `soundbar` == `sound-bar`, `wifi` == `wi-fi`. (Exact-slug matching let `best-smartwatches-2026` slip past `best-smart-watches-2026`.) With `grep -E`, use a plain `|` for OR — `\|` matches a literal pipe and silently returns 0.

Then pick **4 NEW topics** that are NOT in the regenerated index:
